...
function Card({ image, title, description, buttonText, config,onButtonClick,style,imageStyle,columnsJustification,direction='h',scrollable=false,itemsStyle,notificationImage}) {
  return (
    <div style={{ 
        minWidth: '200px',
        margin: '10px', 
    ...
    {image && (
          <div style={{ position: 'relative', width: '100%',...imageStyle }}>
            ...
            <div style={{
              ...
              {notificationImage && <img src={notificationImage} alt="Filter" style={{ height: '50px', width: '50px',borderRadius:'10px',boxShadow: ` -2px -2px 5px rgba(200, 200, 255, 0.3), 3px 3px 5px rgba(0, 0, 0, 0.3)` }} />}
            </div>
          </div>
        )}
